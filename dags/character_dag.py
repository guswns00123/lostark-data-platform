...
def lostark_ark_passive_etl_dag():
    ...
    def extract_all_target_characters(**context) -> str:
        ...
        query = "SELECT distinct character_name FROM lostark.character_info_tb WHERE item_avg_level > 1640;"
        ...
        logging.info(f"데이터 파일 저장 완료: {file_path} (총 {len(extracted_results)}건)")
    ...
    def generate_hourly_summary_report(**kwargs):
        ...
        target_tables = [
            'ark_grid_cores_tb', 'ark_grid_gems_tb', 'ark_passive_effects_tb', 
            'ark_passive_points_tb', 'armory_avatars_tb', 'armory_card_effects_tb', 
            'armory_card_tb', 'armory_collectible_details_tb', 'armory_collectibles_tb', 
            'armory_engravings_tb', 'armory_equipment_tb', 'armory_gem_tb', 
            'armory_profile_tb', 'armory_skills_tb'
        ]
        ...
        report_lines.append(f"✅ **Total Inserted in this hour:** {total_hourly_inserted:,} rows")
